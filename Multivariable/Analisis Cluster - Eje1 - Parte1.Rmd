...
#### Cluster por índice (los dos componentes)
```{r}
fviz_cluster(HC,
             data=Datos[,c(37:38)],
             stand = FALSE,
...
pob_nopobreza, 
sin_ingresoinf, 
PIB_PC, IDH, logevidad, In_marginacion, sinrezago_edu, no_analf, con_eduba, prom_edu, total_sucursales, num_ctas, num_ctas, banca_movil, sinpiso_tierra, in_gini, razon_ingreso, con_ss, sincar_alim, sincar_salud, prop_hospitales, c_covid, incidencia_delic, personal_seg


per_cuarto, , sincar_vivienda, novul_carencias, pob_ocupada, 
...
Tabla1<-Datos %>%
  summarize("n"=n(), 
            "Viviendas con energía" = round(mean(con_energia),1), 
            "Viviendas con agua" =round(mean(con_agua),1),
            "Viviendas con TIC" = round(mean(con_tic),1), 
            "Población no pobre" =round(mean(pob_nopobreza),1),
            "Viviendas con energía" = round(mean(con_energia),1), 
            "Viviendas con agua" =round(mean(con_agua),1),
            "Viviendas con energía" = round(mean(con_energia),1), 
            "Viviendas con agua" =round(mean(con_agua),1),
